...
%             monitor.oldOverrideMultimediaEngine = oldOverrideMultimediaEngine;
    ...
    methods
        ...
        function cycle(obj,run)
            ...
            finalflag = 0; % Initialze finalflag
            endflag = 0; % Initialize keyflag
            t0 = GetSecs;
            if obj.debug
                runLength = 0; % Measuring entire run duration
            end
            tic;
            i = 1;
            ...
            fprintf(fid,'%s,%s,%s,%s,%s,%s,%s,%s\r\n','Scheduled','Reported','Event','Duration','Label','Stimulus','RT','Rating');
            ...
            while ~obj.abort
                ...
                if finalflag % Ignore when final presentation has been set
                else
                    if str2double(tnow)==t % As long as the integer matches
                        ...
                        if i > size(obj.exp.dat{2,run},1) % Signal loop end if index is greater than list size
                            t2 = t + dur; % Override t2
                            finalflag = 1; % Set flag
                        else
                ...
                % Also used to match final presentation duration
                % 1-time occurrence per iteration i, end of presentation
                % trials (not fixation)
                if ~isempty(t2) % If a cut-off fixation is scheduled (or end of loop)
                    
                    tnow2 = regexp(num2str(toc),'\d{1,3}.\d{1}','match','once'); % String conversion of time
                   
                    if str2double(tnow2)==t2 % Match duration cut-off
                        
                        if finalflag % Last presentation
                            
                            if evt % Capture last response if it was a picture
                                % Record response at end of trial
                                fprintf(fid,'%s,%s\r',rt,rate);
                                keyflag = 0;
                            end
                            
                            endflag = 1; % Allow following break (priority 5)
                ...
                % Priority 5: Handling end of run
                if endflag
                    if obj.debug
                        runLength = toc;
                        disp('----'); % Temp break
                        disp(['main.m (cycle) Total run length measured: ' num2str(runLength)]);
                    end
                    break;
                end
